...
def add_low_rank_noise(X, key1, key2, proj_dims = 3, sigma = 0.75, l2_=0.1 ):
    K, N, T = X.shape    
...
np.save(f'{save_path}/{traj}/X_train_sigma{sigma_low_rank}_l2{l2_}', X_train)
np.save(f'{save_path}/{traj}/X_test_sigma{sigma_low_rank}_l2{l2_}', X_test)
...
plt.savefig(f'{save_path}/{traj}/{kernel}/loss_fig_sigma{sigma_low_rank}_l2{l2_}.png')
...
np.save(f'{save_path}/{traj}/{kernel}/params_sigma{sigma_low_rank}_l2{l2_}', params)

np.save(f'{save_path}/{traj}/{kernel}/Y_train_sigma{sigma_low_rank}_l2{l2_}', Y)
...
np.save(f'{save_path}/{traj}/{kernel}/Y_test_sigma{sigma_low_rank}_l2{l2_}', Y)

np.save(f'{save_path}/{traj}/{kernel}/ls_loss_sigma{sigma_low_rank}_l2{l2_}', np.array(ls_loss))
np.save(f'{save_path}/{traj}/{kernel}/ls_S_ratio_sigma{sigma_low_rank}_l2{l2_}', np.array(ls_S_ratio))
...
if linear_run == 'True':
    ...
    plt.figure()
    get_loss_fig(ls_loss, ls_S_ratio)
    plt.savefig(f'{save_path}/{traj}/linear/loss_fig_sigma{sigma_low_rank}_l2{l2_}.png')
    ...
    np.save(f'{save_path}/{traj}/linear/Y_train_sigma{sigma_low_rank}_l2{l2_}', Y)

    U_qr, _ = jnp.linalg.qr(U)        
    Y = jnp.einsum('ji,kjl->kil', U_qr, center(X_test))

    np.save(f'{save_path}/{traj}/linear/Y_test_sigma{sigma_low_rank}_l2{l2_}', Y)

    np.save(f'{save_path}/{traj}/linear/ls_loss_sigma{sigma_low_rank}_l2{l2_}', np.array(ls_loss))
    np.save(f'{save_path}/{traj}/linear/ls_S_ratio_sigma{sigma_low_rank}_l2{l2_}', np.array(ls_S_ratio))
    ...
    np.save(f'{save_path}/{traj}/pca/Y_train_sigma{sigma_low_rank}_l2{l2_}', Y_pca)

    pca = PCA(d)
    Y_pca = pca.fit(X_pca_train).transform(X_pca_test)
    Y_pca = Y_pca.reshape(-1, T, d).swapaxes(1,2)

    np.save(f'{save_path}/{traj}/pca/Y_test_sigma{sigma_low_rank}_l2{l2_}', Y_pca)