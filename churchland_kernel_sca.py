...
X, _ = pre_processing(X_init, center=False, PCA=True)
X = jnp.array(X)
K, N, T = X.shape
A = jnp.swapaxes(pre_processing(X_init, PCA=True)[0], 0, 1)       #(N, K, T)
...
def optimize(P, S, K_A_X, X, iterations=100, learning_rate=0.01, d=3, seed=42):
    K, N, T = X.shape
...
wandb.init(project="SCA-project-kernel", name=f"{FLAGS.name}", mode="online")
optimized_alpha_tilde, ls_loss = optimize(P, S, K_A_X, X, seed = seed )
wandb.finish()
np.save(f"{FLAGS.name}", optimized_alpha_tilde)
