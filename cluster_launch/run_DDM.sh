...
l2_=$(awk -v ArrayTaskID=$SLURM_ARRAY_TASK_ID '$1==ArrayTaskID {print $4}' $config)
proj_dims=$(awk -v ArrayTaskID=$SLURM_ARRAY_TASK_ID '$1==ArrayTaskID {print $5}' $config)

python /rds/user/ar2217/hpc-work/SCA/SCA_project/DDM.py --d $d --sigma_noise $sigma_noise --l2_ $l2_ --proj_dims $proj_dims
